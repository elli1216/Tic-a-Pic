...
export async function GET() {
  try {
    ...
    const { data: defaultStickers, error: defaultError } = await supabaseAnon
      .storage
      .from("stickers")
      .list("default", {
        limit: 100,
    ...
    const { data: uploadedStickers, error: uploadedError } = await supabaseAnon
      .storage
      .from("stickers")
      .list("uploaded", {
        limit: 100,
    ...
    if (uploadedError && uploadedError.message !== "The resource was not found") {
